...
pub unsafe extern "C" fn do_statusbar_prev_word() {
    ...
    while typing_x != 0 as libc::c_int as size_t {
...
pub unsafe extern "C" fn do_statusbar_left() {
    if typing_x > 0 as libc::c_int as size_t {
        typing_x = step_left(answer, typing_x);
        while typing_x > 0 as libc::c_int as size_t
...
pub unsafe extern "C" fn paste_into_answer() {
    ...
    typing_x = typing_x.wrapping_add(pastelen);
}
#[no_mangle]
pub unsafe extern "C" fn do_statusbar_mouse() -> libc::c_int {
            ...
            as libc::c_int != 0
    {
        ...
        if click_row == 0 as libc::c_int && click_col as size_t >= start_col {
            typing_x = actual_x(
                answer,
...
pub unsafe extern "C" fn inject_into_answer(
    ...
    typing_x = typing_x.wrapping_add(count);
}
#[no_mangle]
pub unsafe extern "C" fn do_statusbar_verbatim_input() {
    ...
    if (0 as libc::c_int as size_t) < count && count < 999 as libc::c_int as size_t {
        inject_into_answer(bytes, count);
    } else if count == 0 as libc::c_int as size_t {
        beep();
    }
...
pub unsafe extern "C" fn absorb_character(
    ...
    if function.is_none() {
        if input < 0x20 as libc::c_int && input != '\t' as i32
            || meta_key as libc::c_int != 0 || input > 0xff as libc::c_int
        {
            beep();
...
pub unsafe extern "C" fn get_statusbar_page_start(
    ...
    if column == base || column < (COLS - 1 as libc::c_int) as size_t {
        return 0 as libc::c_int as size_t
    } else if COLS as size_t > base.wrapping_add(2 as libc::c_int as size_t) {
        return column
            .wrapping_sub(base)
            .wrapping_sub(1 as libc::c_int as size_t)
            .wrapping_sub(
                column.wrapping_sub(base).wrapping_sub(1 as libc::c_int as size_t)
                    % (COLS as size_t)
                        .wrapping_sub(base)
                        .wrapping_sub(2 as libc::c_int as size_t),
            )
    } else {
        return column.wrapping_sub(2 as libc::c_int as size_t)
    };
}
#[no_mangle]
...
pub unsafe extern "C" fn draw_the_promptbar() {
    let mut base: size_t = (breadth(prompt)).wrapping_add(2 as libc::c_int as size_t);
    ...
    end_page = get_statusbar_page_start(
        base,
        base.wrapping_add(breadth(answer)).wrapping_sub(1 as libc::c_int as size_t),
    );
    wattr_on(
        footwin,
    ...
    waddch(
        footwin,
        (if the_page == 0 as libc::c_int as size_t { ' ' as i32 } else { '<' as i32 })
            as chtype,
    );
    expanded = display_string(
        answer,
        the_page,
        (COLS as size_t).wrapping_sub(base),
        0 as libc::c_int != 0,
        1 as libc::c_int != 0,
    );
    ...
    if the_page < end_page
        && base.wrapping_add(breadth(answer)).wrapping_sub(the_page) > COLS as size_t
    {
...
pub unsafe extern "C" fn acquire_an_answer(
    ...
    let mut bracketed_paste: bool = 0 as libc::c_int != 0;
    ...
    loop {
        ...
        if input == 0x4f7 as libc::c_int {
            *actual = 0x4f7 as libc::c_int;
            free(stored_string as *mut libc::c_void);
            return None;
        }
        if input == 0x4b5 as libc::c_int || input == 0x4be as libc::c_int {
            bracketed_paste = input == 0x4b5 as libc::c_int;
        }
        ...
        if input == '\t' as i32 && bracketed_paste as libc::c_int != 0 {
            function = None;
        }
        absorb_character(input, function);
        if bracketed_paste {
            if function.is_some()
                && function != Some(do_nothing as unsafe extern "C" fn() -> ())
            {
                beep();
            }
        } else {
            ...
            if function == Some(do_tab as unsafe extern "C" fn() -> ()) {
                if !history_list.is_null() {
                    ...
                    if fragment_length > 0 as libc::c_int as size_t {
                ...
                && (*shortcut).toggle == NO_HELP as libc::c_int
            {
                    ...
                    >(Some(implant as unsafe extern "C" fn(*const libc::c_char) -> ()))
                {
                    ...
                    {
                        if currmenu == (1 as libc::c_int) << 7 as libc::c_int {
    ...
    if currmenu == (1 as libc::c_int) << 7 as libc::c_int
        && function == Some(do_enter as unsafe extern "C" fn() -> ())
    {
        *foretext = '\0' as i32 as libc::c_char;
    }
    if !stored_string.is_null() {
        reset_history_pointer_for(*history_list);
        free(stored_string as *mut libc::c_void);
...
pub unsafe extern "C" fn do_prompt(
    ...
    if function == Some(do_cancel as unsafe extern "C" fn() -> ())
        || function == Some(do_enter as unsafe extern "C" fn() -> ())
        || function == Some(to_first_file as unsafe extern "C" fn() -> ())
...
pub unsafe extern "C" fn ask_user(
    ...
    while choice == -(2 as libc::c_int) {
        ...
        if kbinput == 0x4b5 as libc::c_int {
            kbinput = get_kbinput(footwin, 0 as libc::c_int != 0);
            while get_kbinput(footwin, 0 as libc::c_int != 0) != 0x4be as libc::c_int {}
        }
        let fresh1 = index;
        index = index + 1;
        letter[fresh1 as usize] = kbinput as libc::c_uchar as libc::c_char;
        if 0xc0 as libc::c_int <= kbinput && kbinput <= 0xf7 as libc::c_int
            && using_utf8 as libc::c_int != 0
        {
            ...
            while extras as size_t <= waiting_keycodes()
                && {
                    let fresh2 = extras;
                    extras = extras - 1;
        ...
        {
            choice = 0 as libc::c_int;
            if kbinput != '\u{e}' as i32 {
                final_status = 2 as libc::c_int;
            }
        } else if kbinput == '\u{19}' as i32 {
            choice = 1 as libc::c_int;
        } else if kbinput == '\u{1}' as i32 && withall as libc::c_int != 0 {
            choice = 2 as libc::c_int;
