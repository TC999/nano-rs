...
#[no_mangle]
pub unsafe extern "C" fn expunge(mut action: undo_type) {
        ...
        as libc::c_int != '\0' as i32
    {
        ...
        memmove(
            ...
            &mut *((*(*openfile).current).data)
                .offset(((*openfile).current_x).wrapping_add(charlen as size_t) as isize)
                as *mut libc::c_char as *const libc::c_void,
            line_len
                .wrapping_sub(charlen as size_t)
        ...
        {
            (*openfile).mark_x = ((*openfile).mark_x).wrapping_sub(charlen as size_t);
        }
        ...
        if (*openfile).mark == joining {
            ...
            (*openfile)
                .mark_x = ((*openfile).mark_x).wrapping_add((*openfile).current_x);
        }
        (*(*openfile).current)
...
pub unsafe extern "C" fn do_backspace() {
    ...
    {
        zap_text();
    } else if (*openfile).current_x > 0 as libc::c_int as size_t {
        (*openfile)
...
pub unsafe extern "C" fn is_cuttable(mut test_cliff: bool) -> bool {
    ...
    if ((*(*openfile).current).next).is_null()
        ...
        || (*openfile).mark == (*openfile).current
            && (*openfile).mark_x == (*openfile).current_x
        || from > 0 as libc::c_int as size_t
            && !(flags[(NO_NEWLINES as libc::c_int as libc::c_ulong)
                .wrapping_div(
                    (::core::mem::size_of::<libc::c_uint>() as libc::c_ulong)
                        .wrapping_mul(8 as libc::c_int as libc::c_ulong),
...
pub unsafe extern "C" fn chop_word(mut forward: bool) {
    ...
    if !forward {
        ...
        if (*openfile).current != is_current {
            if is_current_x > 0 as libc::c_int as size_t {
...
pub unsafe extern "C" fn extract_segment(
    ...
    if top == bot {
        ...
        last = taken;
    } else if top_x == 0 as libc::c_int as size_t && bot_x == 0 as libc::c_int as size_t
    {
        taken = top;
    ...
    (*openfile)
        ...
        .wrapping_sub(number_of_characters_in(taken, last));
...
pub unsafe extern "C" fn ingraft_buffer(mut topline: *mut linestruct) {
    ...
    (*openfile)
        .totsize = ((*openfile).totsize)
        .wrapping_add(number_of_characters_in(topline, botline));
    if topline != botline {
        length = xpos;
    }
    if extralen > 0 as libc::c_int as size_t {
        (*line)
            .data = nrealloc(
            (*line).data as *mut libc::c_void,
            length.wrapping_add(extralen).wrapping_add(1 as libc::c_int as size_t),
        ) as *mut libc::c_char;
        memmove(
            ((*line).data).offset(xpos as isize).offset(extralen as isize)
                as *mut libc::c_void,
            ((*line).data).offset(xpos as isize) as *const libc::c_void,
            length.wrapping_sub(xpos).wrapping_add(1 as libc::c_int as size_t),
        );
    ...
    if topline != botline {
        ...
        *((*line).data)
        ...
        (*openfile).current_x = ((*openfile).current_x).wrapping_add(extralen);
    }
    if mark_follows as libc::c_int != 0 && topline != botline {
        (*openfile).mark = botline;
        (*openfile)
            .mark_x = ((*openfile).mark_x).wrapping_add(length.wrapping_sub(xpos));
    } else if mark_follows {
        (*openfile).mark_x = ((*openfile).mark_x).wrapping_add(extralen);
    }
    delete_node(topline);
    free(tailtext as *mut libc::c_void);
    renumber_from(line);
    if !(flags[(NO_NEWLINES as libc::c_int as libc::c_ulong)
        .wrapping_div(
            (::core::mem::size_of::<libc::c_uint>() as libc::c_ulong)
...
pub unsafe extern "C" fn copy_from_buffer(mut somebuffer: *mut linestruct) {
    let mut threshold: size_t = ((*(*openfile).edittop).lineno + editwinrows as ssize_t
        - 1 as libc::c_int as ssize_t) as size_t;
    let mut the_copy: *mut linestruct = copy_buffer(somebuffer);
    ingraft_buffer(the_copy);
    if (*(*openfile).current).lineno as size_t > threshold
