...
pub struct re_pattern_buffer {
    ...
    pub re_nsub: size_t,
    // Simplified bitfield representation - replace with union/packed struct if needed
    pub bitfield_flags: u8,
...
pub unsafe extern "C" fn set_interface_colorpairs() {
    ...
    while index < NUMBER_OF_ELEMENTS as libc::c_int as size_t {
        ...
        if !combo.is_null() {
            ...
            init_pair(
                index.wrapping_add(1 as libc::c_int as size_t) as libc::c_short,
                (*combo).fg,
                (*combo).bg,
            );
            interface_color_pair[index
                as usize] = ((index.wrapping_add(1 as libc::c_int as size_t) as chtype)
                << 0 as libc::c_int + 8 as libc::c_int
                & ((1 as libc::c_uint) << 8 as libc::c_int)
                    .wrapping_sub(1 as libc::c_uint)
                    << 0 as libc::c_int + 8 as libc::c_int
                | (*combo).attributes as chtype) as libc::c_int;
            rescind_colors = 0 as libc::c_int != 0;
        } else if index == FUNCTION_TAG as libc::c_int as size_t
            || index == SCROLL_BAR as libc::c_int as size_t
        {
            interface_color_pair[index
                as usize] = (1 as libc::c_uint).wrapping_sub(1 as libc::c_uint)
                as libc::c_int;
        } else if index == GUIDE_STRIPE as libc::c_int as size_t {
            interface_color_pair[index
                as usize] = ((1 as libc::c_uint) << 10 as libc::c_int + 8 as libc::c_int)
                as libc::c_int;
        } else if index == SPOTLIGHTED as libc::c_int as size_t {
            init_pair(
                index.wrapping_add(1 as libc::c_int as size_t) as libc::c_short,
        ...
        } else if index == MINI_INFOBAR as libc::c_int as size_t
            || index == PROMPT_BAR as libc::c_int as size_t
        {
            interface_color_pair[index
                as usize] = interface_color_pair[TITLE_BAR as libc::c_int as usize];
        } else if index == ERROR_MESSAGE as libc::c_int as size_t {
            init_pair(
                index.wrapping_add(1 as libc::c_int as size_t) as libc::c_short,
        ...
        free(color_combo[index as usize] as *mut libc::c_void);
...
pub unsafe extern "C" fn set_syntax_colorpairs(mut sntx: *mut syntaxtype) {
    ...
    while !ink.is_null() {
        ...
        (*ink)
            .attributes = ((*ink).attributes as chtype
...
pub unsafe extern "C" fn check_the_multis(mut line: *mut linestruct) {
    ...
    if ((*openfile).syntax).is_null()
        || (*(*openfile).syntax).multiscore as libc::c_int == 0 as libc::c_int
    {
        return;
    }
