...
pub unsafe extern "C" fn read_the_list(
    ...
    gauge = widest.wrapping_add(10 as libc::c_int as size_t) as libc::c_int;
...
pub unsafe extern "C" fn reselect(mut name: *const libc::c_char) {
    ...
    if looking_at < list_length {
        ...
        selected = list_length.wrapping_sub(1 as libc::c_int as size_t);
    } else {
...
pub unsafe extern "C" fn browser_refresh() {
    ...
    let mut index: size_t = selected
        .wrapping_sub(selected % (usable_rows * piles as size_t));
    while index < list_length && (row as size_t) < usable_rows {
        ...
        let mut dots: bool = COLS >= 15 as libc::c_int
            && namelen >= (gauge as size_t).wrapping_sub(infomaxlen);
        ...
        if lstat(*filelist.offset(index as isize), &mut state) == -(1 as libc::c_int)
            || state.st_mode & 0o170000 as libc::c_int as __mode_t
                == 0o120000 as libc::c_int as __mode_t
        {
        ...
        } else if state.st_mode & 0o170000 as libc::c_int as __mode_t
            == 0o40000 as libc::c_int as __mode_t
        {
            ...
            info = nmalloc(infomaxlen.wrapping_add(1 as libc::c_int as size_t))
                as *mut libc::c_char;
            if state.st_size < ((1 as libc::c_int) << 10 as libc::c_int) as __off_t {
                modifier = ' ' as i32 as libc::c_char;
            } else if state.st_size
                < ((1 as libc::c_int) << 20 as libc::c_int) as __off_t
            {
                result >>= 10 as libc::c_int;
                modifier = 'K' as i32 as libc::c_char;
            } else if state.st_size
                < ((1 as libc::c_int) << 30 as libc::c_int) as __off_t
            {
            ...
            if result < ((1 as libc::c_int) << 10 as libc::c_int) as off_t {
                sprintf(
                    info,
        ...
        if wmove(midwin, row, (col as size_t).wrapping_sub(infolen) as libc::c_int)
            == -(1 as libc::c_int)
        {
            -(1 as libc::c_int);
        } else {
...
pub unsafe extern "C" fn findfile(mut needle: *const libc::c_char, mut forwards: bool) {
    ...
    loop {
        if forwards {
            ...
            if fresh1 == list_length.wrapping_sub(1 as libc::c_int as size_t) {
...
pub unsafe extern "C" fn search_filename(mut forwards: bool) {
    ...
    if *last_search as libc::c_int != '\0' as i32 {
        ...
        sprintf(
            ...
            b" [%s%s]\0" as *const u8 as *const libc::c_char,
            disp,
            if breadth(last_search) > (COLS / 3 as libc::c_int) as size_t {
                b"...\0" as *const u8 as *const libc::c_char
            } else {
                b"\0" as *const u8 as *const libc::c_char
            },
    ...
    free(thedefault as *mut libc::c_void);
...
pub unsafe extern "C" fn browse(mut path: *mut libc::c_char) -> *mut libc::c_char {
    ...
    '_read_directory_contents: loop {
        ...
        if path.is_null() || dir.is_null() {
            ...
            if filelist.is_null() {
                lastmessage = VACUUM;
                free(present_name as *mut libc::c_void);
        ...
        titlebar(path);
        if list_length == 0 as libc::c_int as size_t {
            statusline(
                ALERT,
                dcgettext(
                    0 as *const libc::c_char,
            ...
            loop {
                ...
                if kbinput == 0o631 as libc::c_int {
                        ...
                        ) as libc::c_int != 0
                    {
                        selected = selected
                            .wrapping_sub(selected % (usable_rows * piles as size_t))
                            .wrapping_add((mouse_y * piles) as size_t)
                            .wrapping_add(
                                (mouse_x / (gauge + 2 as libc::c_int)) as size_t,
                            );
                        ...
                        if selected
                            > list_length.wrapping_sub(1 as libc::c_int as size_t)
                        {
                ...
                if function == Some(do_help as unsafe extern "C" fn() -> ()) {
                    do_help();
                } else if function == Some(full_refresh as unsafe extern "C" fn() -> ())
                {
                    kbinput = 0x4f7 as libc::c_int;
                    ...
                    == Some(do_findprevious as unsafe extern "C" fn() -> ())
                {
                    ...
                    if selected > 0 as libc::c_int as size_t {
                        selected = selected.wrapping_sub(1);
                        selected;
                    }
                } else if function == Some(do_right as unsafe extern "C" fn() -> ()) {
                    if selected < list_length.wrapping_sub(1 as libc::c_int as size_t) {
                        selected = selected.wrapping_add(1);
                        selected;
                    }
                } else if function == Some(to_prev_word as unsafe extern "C" fn() -> ())
                {
                    selected = selected.wrapping_sub(selected % piles as size_t);
                } else if function == Some(to_next_word as unsafe extern "C" fn() -> ())
                {
                    selected = selected
                        .wrapping_add(
                            ((piles - 1 as libc::c_int) as size_t)
                                .wrapping_sub(selected % piles as size_t),
                        );
                    if selected >= list_length {
                        selected = list_length.wrapping_sub(1 as libc::c_int as size_t);
                    }
                } else if function == Some(do_up as unsafe extern "C" fn() -> ()) {
                    if selected >= piles as size_t {
                        selected = selected.wrapping_sub(piles as size_t);
                    }
                } else if function == Some(do_down as unsafe extern "C" fn() -> ()) {
                    if selected.wrapping_add(piles as size_t)
                        <= list_length.wrapping_sub(1 as libc::c_int as size_t)
                    {
                        selected = selected.wrapping_add(piles as size_t);
                    }
                } else if function == Some(to_prev_block as unsafe extern "C" fn() -> ())
                {
                    selected = (selected / (usable_rows * piles as size_t) * usable_rows
                        * piles as size_t)
                        .wrapping_add(selected % piles as size_t);
                } else if function == Some(to_next_block as unsafe extern "C" fn() -> ())
                {
                    selected = (selected / (usable_rows * piles as size_t) * usable_rows
                        * piles as size_t)
                        .wrapping_add(selected % piles as size_t)
                        .wrapping_add(usable_rows * piles as size_t)
                        .wrapping_sub(piles as size_t);
                    if selected >= list_length {
                        selected = (list_length / piles as size_t * piles as size_t)
                            .wrapping_add(selected % piles as size_t);
                    }
                    if selected >= list_length {
                        selected = selected.wrapping_sub(piles as size_t);
                    ...
                    if selected < piles as size_t {
                        selected = 0 as libc::c_int as size_t;
                    } else if selected < usable_rows * piles as size_t {
                        selected = selected % piles as size_t;
                    } else {
                        selected = selected.wrapping_sub(usable_rows * piles as size_t);
                    }
                } else if function == Some(do_page_down as unsafe extern "C" fn() -> ())
                {
                    ...
                    {
                        selected = list_length.wrapping_sub(1 as libc::c_int as size_t);
                    } else if selected.wrapping_add(usable_rows * piles as size_t)
                        >= list_length
                    {
                        selected = (selected
                            .wrapping_add(usable_rows * piles as size_t)
                            .wrapping_sub(list_length) % piles as size_t)
                            .wrapping_add(list_length)
                            .wrapping_sub(piles as size_t);
                    } else {
                        selected = selected.wrapping_add(usable_rows * piles as size_t);
                    }
                } else if function == Some(to_first_file as unsafe extern "C" fn() -> ())
                    || function == Some(to_last_file as unsafe extern "C" fn() -> ())
                {
                ...
                } else if kbinput == 0x4b5 as libc::c_int {
                    while get_kbinput(midwin, 0 as libc::c_int != 0)
                        != 0x4be as libc::c_int
                    {}
                    statusline(
                        AHEM,
                        dcgettext(
                            0 as *const libc::c_char,
                            b"Paste is ignored\0" as *const u8 as *const libc::c_char,
                            5 as libc::c_int,
                        ),
                    );
                } else if !(kbinput == 0x4f7 as libc::c_int) {
                    if function == Some(do_exit as unsafe extern "C" fn() -> ()) {
                        break '_read_directory_contents;
                    }
                    unbound_key(kbinput);
                }
                if !(kbinput == 0x4f7 as libc::c_int || resized_for_browser != 0) {
                    continue;
                }
