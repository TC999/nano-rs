...
pub unsafe extern "C" fn get_edge_and_target(
    ...
    {
        let mut shim: size_t = (editwincols as ssize_t
            * (1 as libc::c_int as ssize_t + tabsize / editwincols as ssize_t))
            as size_t;
        *leftedge = leftedge_for(xplustabs(), (*openfile).current);
        *target_column = ((*openfile).placewewant)
            .wrapping_add(shim)
            .wrapping_sub(*leftedge) % editwincols as size_t;
    } else {
...
pub unsafe extern "C" fn proper_x(
    ...
    if flags[(SOFTWRAP as libc::c_int as libc::c_ulong)
        ...
        && (forward as libc::c_int != 0 && wideness((*line).data, index) < *leftedge
            || !forward
                && column / tabsize as size_t
                    == (*leftedge).wrapping_sub(1 as libc::c_int as size_t)
                        / tabsize as size_t
                && (column / tabsize as size_t)
                    < (*leftedge)
                        .wrapping_add(editwincols as size_t)
                        .wrapping_sub(1 as libc::c_int as size_t) / tabsize as size_t)
    {
...
#[no_mangle]
pub unsafe extern "C" fn to_top_row() {
    let mut leftedge: size_t = 0;
    let mut offset: size_t = 0;
    get_edge_and_target(&mut leftedge, &mut offset);
    (*openfile).current = (*openfile).edittop;
    leftedge = (*openfile).firstcolumn;
    set_proper_index_and_pww(&mut leftedge, offset, 0 as libc::c_int != 0);
    refresh_needed = !((*openfile).mark).is_null();
}
#[no_mangle]
pub unsafe extern "C" fn to_bottom_row() {
    let mut leftedge: size_t = 0;
    let mut offset: size_t = 0;
    get_edge_and_target(&mut leftedge, &mut offset);
    (*openfile).current = (*openfile).edittop;
    ...
    set_proper_index_and_pww(&mut leftedge, offset, 1 as libc::c_int != 0);
    refresh_needed = !((*openfile).mark).is_null();
}
#[no_mangle]
pub unsafe extern "C" fn do_cycle() {
    if cycling_aim == 0 as libc::c_int {
        adjust_viewport(CENTERING);
    } else {
        (*openfile)
            .cursor_row = (if cycling_aim == 1 as libc::c_int {
            0 as libc::c_int
        } else {
            editwinrows - 1 as libc::c_int
        }) as ssize_t;
    ...
    cycling_aim = (cycling_aim + 1 as libc::c_int) % 3 as libc::c_int;
    draw_all_subwindows();
    full_refresh();
}
#[no_mangle]
...
pub unsafe extern "C" fn do_up() {
    ...
    set_proper_index_and_pww(&mut leftedge, target_column, 0 as libc::c_int != 0);
    if (*openfile).cursor_row == 0 as libc::c_int as ssize_t
        ...
        && (tabsize < editwincols as ssize_t
            || !(flags[(SOFTWRAP as libc::c_int as libc::c_ulong)
                .wrapping_div(
                    (::core::mem::size_of::<libc::c_uint>() as libc::c_ulong)
                        .wrapping_mul(8 as libc::c_int as libc::c_ulong),
...
pub unsafe extern "C" fn do_left() {
    ...
    if (*openfile).current_x > 0 as libc::c_int as size_t {
        (*openfile)
            .current_x = step_left((*(*openfile).current).data, (*openfile).current_x);
        while (*openfile).current_x > 0 as libc::c_int as size_t
            && is_zerowidth(
                ((*(*openfile).current).data).offset((*openfile).current_x as isize),
            ) as libc::c_int != 0
        {
