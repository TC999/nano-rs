...
pub const EMERGENCY: kind_of_writing_type = 3;
...
pub unsafe extern "C" fn do_tab() {
    if !((*openfile).mark).is_null() && (*openfile).mark != (*openfile).current {
        do_indent();
    } else if !((*openfile).syntax).is_null()
        && !((*(*openfile).syntax).tabstring).is_null()
    {
                ...
                ) != 0 as libc::c_int as libc::c_uint
    {
        let mut spaces: *mut libc::c_char = nmalloc(
            (tabsize + 1 as libc::c_int as ssize_t) as size_t,
        ) as *mut libc::c_char;
        let mut length: size_t = (tabsize as size_t)
            .wrapping_sub(xplustabs() % tabsize as size_t);
...
pub unsafe extern "C" fn indent_a_line(
    ...
    if indent_len == 0 as libc::c_int as size_t {
        return;
    }
    (*line)
        .data = nrealloc(
        (*line).data as *mut libc::c_void,
        length.wrapping_add(indent_len).wrapping_add(1 as libc::c_int as size_t),
    ) as *mut libc::c_char;
    memmove(
    ...
    (*openfile).totsize = ((*openfile).totsize).wrapping_add(indent_len);
    if line == (*openfile).mark && (*openfile).mark_x > 0 as libc::c_int as size_t {
        (*openfile).mark_x = ((*openfile).mark_x).wrapping_add(indent_len);
    }
    if line == (*openfile).current && (*openfile).current_x > 0 as libc::c_int as size_t
    {
        (*openfile).current_x = ((*openfile).current_x).wrapping_add(indent_len);
...
pub unsafe extern "C" fn do_indent() {
    ...
    indentation = nmalloc((tabsize + 1 as libc::c_int as ssize_t) as size_t)
        as *mut libc::c_char;
    if !((*openfile).syntax).is_null() && !((*(*openfile).syntax).tabstring).is_null() {
    ...
    free(indentation as *mut libc::c_void);
    set_modified();
...
pub unsafe extern "C" fn length_of_white(mut text: *const libc::c_char) -> size_t {
    ...
    if !((*openfile).syntax).is_null() && !((*(*openfile).syntax).tabstring).is_null() {
        let mut thelength: size_t = strlen((*(*openfile).syntax).tabstring);
        while *text.offset(white_count as isize) as libc::c_int
            == *((*(*openfile).syntax).tabstring).offset(white_count as isize)
                as libc::c_int
        {
    ...
    loop {
        if *text as libc::c_int == '\t' as i32 {
            return white_count.wrapping_add(1 as libc::c_int as size_t);
        }
        if *text as libc::c_int != ' ' as i32 {
            return white_count;
        }
        white_count = white_count.wrapping_add(1);
        if white_count == tabsize as size_t {
...
pub unsafe extern "C" fn compensate_leftward(
    ...
    if line == (*openfile).mark {
        if (*openfile).mark_x < leftshift {
            ...
            (*openfile).mark_x = ((*openfile).mark_x).wrapping_sub(leftshift);
        }
    }
    if line == (*openfile).current {
        if (*openfile).current_x < leftshift {
            ...
            (*openfile).current_x = ((*openfile).current_x).wrapping_sub(leftshift);
        }
        (*openfile).placewewant = xplustabs();
    }
}
...
pub unsafe extern "C" fn unindent_a_line(
    ...
    memmove(
        ...
        length.wrapping_sub(indent_len).wrapping_add(1 as libc::c_int as size_t),
    );
    (*openfile).totsize = ((*openfile).totsize).wrapping_sub(indent_len);
...
pub unsafe extern "C" fn do_unindent() {
    ...
    get_range(&mut top, &mut bot);
    while top != (*bot).next
        && length_of_white((*top).data) == 0 as libc::c_int as size_t
    {
        top = (*top).next;
    }
    if top == (*bot).next {
...
pub unsafe extern "C" fn comment_line(
    ...
    let mut pre_len: size_t = if !post_seq.is_null() {
        ...
        fresh0.offset_from(comment_seq) as libc::c_long as size_t
    } else {
        comment_seq_len
    };
    let mut post_len: size_t = if !post_seq.is_null() {
        comment_seq_len.wrapping_sub(pre_len).wrapping_sub(1 as libc::c_int as size_t)
    } else {
    ...
    if action as libc::c_uint == COMMENT as libc::c_int as libc::c_uint {
        ...
        memmove(
            ...
            (*line).data as *const libc::c_void,
            line_len.wrapping_add(1 as libc::c_int as size_t),
        );
        memmove(
            (*line).data as *mut libc::c_void,
        ...
        if post_len > 0 as libc::c_int as size_t {
            memmove(
                ((*line).data).offset(pre_len as isize).offset(line_len as isize)
                    as *mut libc::c_void,
                post_seq as *const libc::c_void,
                post_len.wrapping_add(1 as libc::c_int as size_t),
            );
        }
        (*openfile)
            .totsize = ((*openfile).totsize)
            .wrapping_add(pre_len.wrapping_add(post_len));
        if line == (*openfile).mark && (*openfile).mark_x > 0 as libc::c_int as size_t {
            (*openfile).mark_x = ((*openfile).mark_x).wrapping_add(pre_len);
        }
        if line == (*openfile).current
            && (*openfile).current_x > 0 as libc::c_int as size_t
        {
            (*openfile).current_x = ((*openfile).current_x).wrapping_add(pre_len);
            (*openfile).placewewant = xplustabs();
        }
        return 1 as libc::c_int != 0;
    }
    if strncmp((*line).data, comment_seq, pre_len) == 0 as libc::c_int
        && (post_len == 0 as libc::c_int as size_t
            || strcmp(
            ...
            ) == 0 as libc::c_int)
    {
        ...
        *((*line).data)
        ...
        (*openfile)
            .totsize = ((*openfile).totsize)
            .wrapping_sub(pre_len.wrapping_add(post_len));
...
pub unsafe extern "C" fn do_undo() {
    ...
    match (*u).type_0 as libc::c_uint {
        ...
        1 => {
            ...
            original_x = if (*u).head_x == 0 as libc::c_int as size_t {
                (*u).tail_x
            } else {
                (*u).head_x
            };
            regain_from_x = if (*u).head_x == 0 as libc::c_int as size_t {
                0 as libc::c_int as size_t
            } else {
                (*u).tail_x
            };
            ...
            (*line)
                ...
                | (*(*line).next).has_anchor as libc::c_int) as bool;
        ...
        2 | 3 => {
            ...
            free((*line).data as *mut libc::c_void);
            (*line).data = data;
            goto_line_posx((*u).tail_lineno, (*u).tail_x);
        }
        4 => {
    ...
    if (*u).type_0 as libc::c_uint <= REPLACE as libc::c_int as libc::c_uint {
        ...
        || (*u).type_0 as libc::c_uint == COUPLE_BEGIN as libc::c_int as libc::c_uint
    {
...
pub unsafe extern "C" fn do_redo() {
    ...
    match (*u).type_0 as libc::c_uint {
        ...
        1 => {
            ...
            *((*line).data).offset((*u).head_x as isize) = '\0' as i32 as libc::c_char;
            ...
            splice_node(line, intruder);
            renumber_from(intruder);
            goto_line_posx((*u).head_lineno + 1 as libc::c_int as ssize_t, (*u).tail_x);
        }
        2 | 3 => {
            redidmsg = dcgettext(
                0 as *const libc::c_char,
    ...
    if (*u).type_0 as libc::c_uint <= REPLACE as libc::c_int as libc::c_uint {
        ...
        || (*u).type_0 as libc::c_uint == COUPLE_END as libc::c_int as libc::c_uint
    {
        recook = 1 as libc::c_int != 0;
    }
    if (*openfile).current_undo == (*openfile).last_saved {
...
pub unsafe extern "C" fn do_enter() {
        ...
        && (*openfile).mark_x > (*openfile).current_x
    {
        ...
        (*openfile)
            .mark_x = ((*openfile).mark_x)
            .wrapping_add(extra.wrapping_sub((*openfile).current_x));
    }
    if flags[(AUTOINDENT as libc::c_int as libc::c_ulong)
        .wrapping_div(
            (::core::mem::size_of::<libc::c_uint>() as libc::c_ulong)
    ...
    *((*(*openfile).current).data)
    ...
    {
        (*openfile).totsize = ((*openfile).totsize).wrapping_add(extra);
    }
    update_undo(ENTER);
    refresh_needed = 1 as libc::c_int != 0;
    focusing = 0 as libc::c_int != 0;
...
pub unsafe extern "C" fn discard_until(mut thisitem: *const undostruct) {
    ...
    while !dropit.is_null() && dropit != thisitem as *mut undostruct {
        ...
        free((*dropit).strdata as *mut libc::c_void);
        ...
        while !group.is_null() {
            ...
            free_chararray(
                (*group).indentations,
                ((*group).bottom_line - (*group).top_line + 1 as libc::c_int as ssize_t)
                    as size_t,
            );
            free(group as *mut libc::c_void);
            group = next;
        }
        free(dropit as *mut libc::c_void);
...
pub unsafe extern "C" fn add_undo(
    ...
    let mut u: *mut undostruct = nmalloc(
        ::core::mem::size_of::<undostruct>() as libc::c_ulong,
    ) as *mut undostruct;
    ...
    match (*u).type_0 as libc::c_uint {
        0 => {
            ...
            current_block_67 = 3392087639489470149;
        }
        2 => {
            ...
            current_block_67 = 9137367745596032277;
        }
        3 => {
            current_block_67 = 9137367745596032277;
        }
        ...
        17 => {
            ...
            current_block_67 = 6346722860260143362;
        }
        18 => {
            current_block_67 = 6346722860260143362;
        }
        19 => {
            ...
            current_block_67 = 7940009801594355625;
        }
        20 => {
            current_block_67 = 7940009801594355625;
        }
        ...
        9137367745596032277 => {
                ...
                != '\0' as i32
            {
                ...
                if (*u).type_0 as libc::c_uint == BACK as libc::c_int as libc::c_uint {
                    (*u).tail_x = ((*u).tail_x).wrapping_add(charlen as size_t);
                }
            } else {
                action = JOIN;
...
pub unsafe extern "C" fn update_multiline_undo(
    ...
    if !((*u).grouping).is_null()
        && (*(*u).grouping).bottom_line + 1 as libc::c_int as ssize_t == lineno
    {
        let mut number_of_lines: size_t = (lineno - (*(*u).grouping).top_line
            + 1 as libc::c_int as ssize_t) as size_t;
        ...
        let ref mut fresh1 = *((*(*u).grouping).indentations)
            .offset(number_of_lines.wrapping_sub(1 as libc::c_int as size_t) as isize);
        *fresh1 = copy_of(indentation);
    } else {
        let mut born: *mut groupstruct = nmalloc(
            ::core::mem::size_of::<groupstruct>() as libc::c_ulong,
        ) as *mut groupstruct;
    ...
    match (*u).type_0 as libc::c_uint {
        0 => {
            ...
            (*u)
                ...
                newlen.wrapping_add(1 as libc::c_int as size_t),
            ) as *mut libc::c_char;
            strncpy(
                (*u).strdata,
        ...
        2 | 3 => {
            ...
            if (*openfile).current_x == (*u).head_x {
                (*u)
                    ...
                    (*u).strdata as *mut libc::c_void,
                    datalen
                        .wrapping_add(charlen as size_t)
                        .wrapping_add(1 as libc::c_int as size_t),
                ) as *mut libc::c_char;
                strncpy(
                ...
                *((*u).strdata)
                    .offset(
                        datalen.wrapping_add(charlen as size_t) as isize,
                    ) = '\0' as i32 as libc::c_char;
                (*u).tail_x = (*openfile).current_x;
            } else if (*openfile).current_x
                == ((*u).head_x).wrapping_sub(charlen as size_t)
            {
                (*u)
                    .strdata = nrealloc(
                    (*u).strdata as *mut libc::c_void,
                ...
                memmove(
                    ...
                    datalen.wrapping_add(1 as libc::c_int as size_t),
                );
            ...
            match current_block_46 {
                ...
                _ => {
                    if (*u).xflags & (1 as libc::c_int) << 4 as libc::c_int == 0 {
                        ...
                        (*u)
                            .tail_lineno = ((*u).head_lineno as size_t)
                                ...
                                == CUT_TO_EOF as libc::c_int as libc::c_uint
                        {
                            ...
                            if count == 0 as libc::c_int as size_t {
                                (*u).tail_x = ((*u).tail_x).wrapping_add((*u).head_x);
                            }
...
pub unsafe extern "C" fn do_wrap() {
    ...
    if wrap_loc < 0 as libc::c_int as ssize_t
        || lead_len.wrapping_add(wrap_loc as size_t) == line_len
    {
        return;
    }
    wrap_loc = lead_len
    ...
    if quot_len > 0 as libc::c_int as size_t {
    ...
    rest_length = line_len.wrapping_sub(wrap_loc as size_t);
        ...
        && rest_length.wrapping_add(breadth((*(*line).next).data)) <= wrap_at
    {
        ...
        if !is_blank_char(remainder.offset(step_left(remainder, rest_length) as isize)) {
            ...
            (*line)
                .data = nrealloc(
                (*line).data as *mut libc::c_void,
                line_len.wrapping_add(2 as libc::c_int as size_t),
            ) as *mut libc::c_char;
            *((*line).data).offset(line_len as isize) = ' ' as i32 as libc::c_char;
            *((*line).data)
                .offset(
                    line_len.wrapping_add(1 as libc::c_int as size_t) as isize,
                ) = '\0' as i32 as libc::c_char;
                ...
                ) != 0 as libc::c_int as libc::c_uint
    {
        ...
        while (rear_x != typed_x || cursor_x >= wrap_loc as size_t)
    ...
    if quot_len > 0 as libc::c_int as size_t {
        ...
        (*line)
            .data = nrealloc(
            (*line).data as *mut libc::c_void,
            lead_len.wrapping_add(line_len).wrapping_add(1 as libc::c_int as size_t),
        ) as *mut libc::c_char;
        memmove(
        ...
        (*openfile).current_x = ((*openfile).current_x).wrapping_add(lead_len);
        (*openfile).totsize = ((*openfile).totsize).wrapping_add(lead_len);
        free((*(*openfile).undotop).strdata as *mut libc::c_void);
        update_undo(ENTER);
        if autowhite {
            flags[(AUTOINDENT as libc::c_int as libc::c_ulong)
                .wrapping_div(
    ...
    if cursor_x < wrap_loc as size_t {
        (*openfile).current = (*(*openfile).current).prev;
        (*openfile).current_x = cursor_x;
    } else {
        (*openfile)
            .current_x = ((*openfile).current_x)
            .wrapping_add(cursor_x.wrapping_sub(wrap_loc as size_t));
    }
    (*openfile).placewewant = xplustabs();
...
pub unsafe extern "C" fn break_line(
    ...
    while *pointer as libc::c_int != '\0' as i32 && column as ssize_t <= goal {
        if is_blank_char(pointer) as libc::c_int != 0
            && (!inhelp || column > 17 as libc::c_int as size_t
                || goal < 40 as libc::c_int as ssize_t)
        {
            lastblank = pointer;
...
pub unsafe extern "C" fn quote_length(mut line: *const libc::c_char) -> size_t {
    ...
    if rc == _REG_NOMATCH as libc::c_int || matches.rm_so == -(1 as libc::c_int) {
...
pub unsafe extern "C" fn find_paragraph(
    ...
    *linecount = ((*line).lineno - (**firstline).lineno + 1 as libc::c_int as ssize_t)
        as size_t;
    return 1 as libc::c_int != 0;
}
#[no_mangle]
pub unsafe extern "C" fn concat_paragraph(mut line: *mut linestruct, mut count: size_t) {
    while count > 1 as libc::c_int as size_t {
        ...
        let mut line_len: size_t = strlen((*line).data);
        if line_len > 0 as libc::c_int as size_t
            && *((*line).data)
                .offset(line_len.wrapping_sub(1 as libc::c_int as size_t) as isize)
        ...
        (*line)
            .has_anchor = ((*line).has_anchor as libc::c_int
            | (*next_line).has_anchor as libc::c_int) as bool;
...
pub unsafe extern "C" fn rewrap_paragraph(
    ...
    while breadth((**line).data) > wrap_at {
        ...
        if break_pos < 0 as libc::c_int as ssize_t
            || lead_len.wrapping_add(break_pos as size_t) == line_len
        {
            break;
        }
        break_pos = (break_pos as size_t)
            .wrapping_add(lead_len.wrapping_add(1 as libc::c_int as size_t)) as ssize_t
            as ssize_t;
        ...
        (*(**line).next)
            ...
            lead_len
                ...
                .wrapping_sub(break_pos as size_t)
                .wrapping_add(1 as libc::c_int as size_t),
        ) as *mut libc::c_char;
        ...
        {
            while break_pos > 0 as libc::c_int as ssize_t
                && *((**line).data)
                    .offset((break_pos - 1 as libc::c_int as ssize_t) as isize)
                    as libc::c_int == ' ' as i32
            {
                break_pos -= 1;
                break_pos;
    ...
    if (**line).lineno >= editwinrows as ssize_t {
        recook = 1 as libc::c_int != 0;
    }
...
pub unsafe extern "C" fn justify_paragraph(
    ...
    sampleline = if count == 1 as libc::c_int as size_t { *line } else { (**line).next };
    ...
    free(lead_string as *mut libc::c_void);
}
#[no_mangle]
pub unsafe extern "C" fn justify_text(mut whole_buffer: bool) {
    ...
    let mut marked_backward: bool = !((*openfile).mark).is_null()
    ...
    if !((*openfile).mark).is_null() {
        ...
        while start_x > 0 as libc::c_int as size_t
            && is_blank_char(
                &mut *((*startline).data)
                    .offset(start_x.wrapping_sub(1 as libc::c_int as size_t) as isize),
            ) as libc::c_int != 0
        {
        ...
        if (0 as libc::c_int as size_t) < end_x && end_x < fore_len {
            end_x = fore_len;
        }
        while end_x > 0 as libc::c_int as size_t
        ...
        secondary_lead = nmalloc(secondary_len.wrapping_add(1 as libc::c_int as size_t))
            as *mut libc::c_char;
        ...
        *secondary_lead.offset(secondary_len as isize) = '\0' as i32 as libc::c_char;
        ...
        linecount = ((*endline).lineno - (*startline).lineno
            + (if end_x > 0 as libc::c_int as size_t {
                1 as libc::c_int
            } else {
                0 as libc::c_int
            }) as ssize_t) as size_t;
    ...
    if !((*openfile).mark).is_null() {
        ...
        if fore_len_0 > 0 as libc::c_int as size_t {
            memmove(
                (*line).data as *mut libc::c_void,
                ((*line).data).offset(fore_len_0 as isize) as *const libc::c_void,
                text_len.wrapping_add(1 as libc::c_int as size_t),
            );
        }
        if primary_len > 0 as libc::c_int as size_t {
            (*line)
                .data = nrealloc(
                (*line).data as *mut libc::c_void,
                primary_len
        ...
        if start_x > 0 as libc::c_int as size_t {
        ...
        if end_x > 0 as libc::c_int as size_t && before_eol as libc::c_int != 0 {
            (*line).next = make_new_node(line);
            (*(*line).next).data = copy_of(primary_lead);
        }
        free(secondary_lead as *mut libc::c_void);
        free(primary_lead as *mut libc::c_void);
        focusing = 0 as libc::c_int != 0;
    } else {
        jusline = cutbuffer;
...
pub unsafe extern "C" fn treat(
    ...
    if stat(tempfile_name, &mut fileinfo) == 0 as libc::c_int {
        if fileinfo.st_size == 0 as libc::c_int as __off_t {
    ...
    if thepid < 0 as libc::c_int {
        ...
        free(*arguments.offset(0 as libc::c_int as isize) as *mut libc::c_void);
        return;
...
pub unsafe extern "C" fn do_int_speller(mut tempfile_name: *const libc::c_char) {
    ...
    loop {
        ...
        if !(bytesread > 0 as libc::c_int as size_t) {
            break;
        }
        totalread = totalread.wrapping_add(bytesread);
        buffersize = buffersize.wrapping_add(pipesize as size_t);
    ...
    *pointer = '\0' as i32 as libc::c_char;
    ...
    free(misspellings as *mut libc::c_void);
    refresh_needed = 1 as libc::c_int != 0;
    memcpy(
...
pub unsafe extern "C" fn do_linter() {
    ...
    *pointer = '\0' as i32 as libc::c_char;
                ...
                if !filename.is_null() && !spacer.is_null() {
                    ...
                    if !linestring.is_null() {
                        ...
                        if !colstring.is_null() {
                            ...
                            if linenumber <= 0 as libc::c_int as ssize_t {
                                free(complaint as *mut libc::c_void);
                                pointer = pointer.offset(1);
                                pointer;
                                continue;
                            } else {
                                if colnumber <= 0 as libc::c_int as ssize_t {
                                ...
                                tmplint = curlint;
                                curlint = nmalloc(
                                    ::core::mem::size_of::<lintstruct>() as libc::c_ulong,
                                ) as *mut lintstruct;
    ...
    free(lintings as *mut libc::c_void);
        ...
        || (lint_status & 0xff00 as libc::c_int) >> 8 as libc::c_int > 2 as libc::c_int
    {
        ...
        while !curlint.is_null() {
            ...
            free((*tmplint).msg as *mut libc::c_void);
            free((*tmplint).filename as *mut libc::c_void);
            free(tmplint as *mut libc::c_void);
        }
        return;
    }
    if !parsesuccess {
    ...
    loop {
                ...
                || (*(*openfile).statinfo).st_ino != lintfileinfo.st_ino)
        {
                ...
                || (*(*openfile).statinfo).st_ino != lintfileinfo.st_ino
            {
                ...
                if choice_0 == -(1 as libc::c_int) {
                    ...
                    free(dontwantfile as *mut libc::c_void);
                    if restlint.is_null() {
                        statusline(
                            REMARK,
                            dcgettext(
        ...
        if tmplint != curlint {
            goto_line_posx(
                (*curlint).lineno,
                ((*curlint).colno - 1 as libc::c_int as ssize_t) as size_t,
            );
            (*openfile)
                .current_x = actual_x(
                (*(*openfile).current).data,
        ...
        if kbinput == 0x4f7 as libc::c_int {
            continue;
        }
...
pub unsafe extern "C" fn count_lines_words_and_characters() {
    ...
    lines
        += (if bot_x == 0 as libc::c_int as size_t
...
pub unsafe extern "C" fn do_verbatim_input() {
    ...
    if flags[(ZERO as libc::c_int as libc::c_ulong)
        ...
        && (*openfile).cursor_row == (editwinrows - 1 as libc::c_int) as ssize_t
...
pub unsafe extern "C" fn copy_completion(
    ...
    word = nmalloc(length.wrapping_add(1 as libc::c_int as size_t)) as *mut libc::c_char;
    while index < length {
        let fresh12 = text;
        text = text.offset(1);
        let fresh13 = index;
...
pub unsafe extern "C" fn complete_a_word() {
    ...
    if pletion_line.is_null() {
        while !list_of_completions.is_null() {
            ...
            free((*dropit).word as *mut libc::c_void);
            free(dropit as *mut libc::c_void);
        }
        (*openfile).last_action = OTHER;
        scouring = openfile;
    ...
    while start_of_shard > 0 as libc::c_int as size_t {
    ...
    *shard.offset(shard_length as isize) = '\0' as i32 as libc::c_char;
    while !pletion_line.is_null() {
        let mut threshold: ssize_t = (strlen((*pletion_line).data))
            .wrapping_sub(shard_length) as ssize_t;
                            ...
                            ) as libc::c_int != 0)
                        {
                                ...
                                && i == ((*openfile).current_x).wrapping_sub(shard_length))
                            {
                                ...
                                if !some_word.is_null() {
                                    free(completion as *mut libc::c_void);
                                } else {
                                    some_word = nmalloc(
                                        ::core::mem::size_of::<completionstruct>() as libc::c_ulong,
                                    ) as *mut completionstruct;
