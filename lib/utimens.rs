...
#[inline]
unsafe extern "C" fn issymlink(mut filename: *const libc::c_char) -> libc::c_int {
    let mut linkbuf: [libc::c_char; 1] = [0; 1];
    if readlink(
        filename,
        linkbuf.as_mut_ptr(),
...
unsafe extern "C" fn is_valid_timespec(mut timespec: *const timespec) -> bool {
    return (*timespec).tv_nsec
        ...
        || 0 as libc::c_int as __syscall_slong_t <= (*timespec).tv_nsec
            && (*timespec).tv_nsec < TIMESPEC_HZ as libc::c_int as __syscall_slong_t;
}
unsafe extern "C" fn is_valid_timespecs(mut timespec: *const timespec) -> bool {
    return is_valid_timespec(&*timespec.offset(0 as libc::c_int as isize)) as libc::c_int
        != 0
        && is_valid_timespec(&*timespec.offset(1 as libc::c_int as isize)) as libc::c_int
            != 0;
}
unsafe extern "C" fn validate_timespec(mut timespec: *mut timespec) -> libc::c_int {
    ...
    if !is_valid_timespecs(timespec as *const timespec) {
...
pub unsafe extern "C" fn fdutimens(
        ...
        timeval[0 as libc::c_int
            ...
            let mut init = timeval {
                ...
                tv_usec: (*ts.offset(0 as libc::c_int as isize)).tv_nsec
                    / 1000 as libc::c_int as __syscall_slong_t,
            };
            init
        };
        timeval[1 as libc::c_int
            as usize] = {
            let mut init = timeval {
    ...
    {
        if !t.is_null() {
            let mut abig: bool = 500000 as libc::c_int as __suseconds_t
                <= (*t.offset(0 as libc::c_int as isize)).tv_usec;
            let mut mbig: bool = 500000 as libc::c_int as __suseconds_t
                ...
                && fstat(fd, &mut st) == 0 as libc::c_int
            {
                ...
                if abig as libc::c_int != 0 && adiff == 1 as libc::c_int as time_t
                ...
                if mbig as libc::c_int != 0 && mdiff == 1 as libc::c_int as time_t
...
pub unsafe extern "C" fn lutimens(
    ...
    let mut not_symlink: bool = false;
    if adjustment_needed != 0 || 0 as libc::c_int != 0 {
        not_symlink = !(st.st_mode & 0o170000 as libc::c_int as __mode_t
            == 0o120000 as libc::c_int as __mode_t);
    } else {
        let mut ret: libc::c_int = issymlink(file);
