...
pub unsafe extern "C" fn mfile_name_concat(
    ...
    let mut dirlen: size_t = (dirbase.offset_from(dir) as libc::c_long as size_t)
        .wrapping_add(dirbaselen);
    let mut baselen: size_t = strlen(base);
    let mut sep: libc::c_char = '\0' as i32 as libc::c_char;
    if dirbaselen != 0 {
        if !(*dir.offset(dirlen.wrapping_sub(1 as libc::c_int as size_t) as isize)
    ...
    let mut p_concat: *mut libc::c_char = malloc(
        dirlen
            .wrapping_add((sep as libc::c_int != '\0' as i32) as libc::c_int as size_t)
