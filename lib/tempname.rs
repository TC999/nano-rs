...
unsafe extern "C" fn mix_random_values(
    ...
    return (2862933555777941757 as libc::c_long as random_value * r)
        .wrapping_add(3037000493 as libc::c_long as random_value) ^ s;
}
...
pub unsafe extern "C" fn try_tempname_len(
    ...
    len = strlen(tmpl);
    if len < x_suffix_len.wrapping_add(suffixlen as size_t)
        || strspn(
            &mut *tmpl
                .offset(
                    len.wrapping_sub(x_suffix_len).wrapping_sub(suffixlen as size_t)
                        as isize,
                ),
    ...
    XXXXXX = &mut *tmpl
        .offset(
            len.wrapping_sub(x_suffix_len).wrapping_sub(suffixlen as size_t) as isize,
        ) as *mut libc::c_char;
    ...
    while count < attempts {
        ...
        while i < x_suffix_len {
            ...
            *XXXXXX
                .offset(
                    i as isize,
                ) = letters[(vdigbuf % 62 as libc::c_int as random_value) as usize];
            vdigbuf = vdigbuf / 62 as libc::c_int as random_value;
            vdigits -= 1;
            vdigits;
            i = i.wrapping_add(1);
            i;
