...
pub unsafe extern "C" fn base_len(mut name: *const libc::c_char) -> size_t {
    ...
    len = strlen(name);
    while (1 as libc::c_int as size_t) < len
        && *name.offset(len.wrapping_sub(1 as libc::c_int as size_t) as isize)
            as libc::c_int == '/' as i32
    {
        len = len.wrapping_sub(1);
        len;
    }
    if 0 as libc::c_int != 0 && len == 1 as libc::c_int as size_t
    ...
    {
        return prefix_len.wrapping_add(1 as libc::c_int as size_t);
    }
    return len;
}
